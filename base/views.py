...
def provResultPage(request, prov_title, prov_id, quiz_id):
    ...
    quiz_1_wrong_count = 40 - quiz_1_correct_count
    quiz_2_wrong_count = 40 - quiz_2_correct_count
    quiz_3_wrong_count = 40 - quiz_3_correct_count
    quiz_4_wrong_count = 40 - quiz_4_correct_count
...
def övningsBanken(request):
    ...
    if request.method == 'POST':
        ...
        if formtype == 'kvantitativ':
            övningar_lst = []
            clicked = 'kvantitativ'
            for quiz in alla_övningar:
                if 'vantitativ' in quiz.name:
                    övningar_lst.append(quiz)
